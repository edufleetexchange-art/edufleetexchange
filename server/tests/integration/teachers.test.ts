...
function signCookie(accountId: string, role: string): string {
  const token = jwt.sign({ accountId, role }, JWT_CONFIG.secret, { expiresIn: '1h' });
  return `token=${token}`;
}
...
beforeEach(async () => {
  ...
  // Listing teachers is authenticated now (no anonymous PII enumeration).
  // Sign in as a viewer teacher so the existing filter assertions can run.
  const viewer = await authService.signupTeacher({
    name: 'Viewer', email: 'viewer@e.com', password: 'pwpwpw',
    experience: 1, qualifications: [], subjects: ['General'],
  });
  cookie = signCookie(String(viewer.account.id ?? viewer.account._id), 'teacher');
});
...
describe('GET /api/teachers', () => {
  it('returns teachers filtered by subject', async () => {
    ...
    const res = await request(app).get('/api/teachers?subject=Math').set('Cookie', cookie);
  ...
  it('returns teachers filtered by minimum experience', async () => {
    ...
    const res = await request(app).get('/api/teachers?minExperience=5').set('Cookie', cookie);
    expect(res.status).toBe(200);
    expect(res.body.data.items.length).toBe(1);
    expect(res.body.data.items[0].account.name).toBe('H');
  });

  it('rejects anonymous callers — no PII enumeration', async () => {
